...
import { searchVehicle } from './apiServices.js'; // Make sure this path is correct
...
button.addEventListener('click', async () => {
  ...
  chatbox.innerHTML += `<div class="user-message">Searching for: ${plate}</div>`;

  try {
    const result = await searchVehicle(plate);

    // Display result in chatbox
    chatbox.innerHTML += `<div class="bot-message"><pre>${JSON.stringify(result, null, 2)}</pre></div>`;
  } catch (err) {
    chatbox.innerHTML += `<div class="bot-message error">Error: Could not find vehicle.</div>`;
  }
});
